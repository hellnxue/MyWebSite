...
$(document).ready(function(){
	...
	$(".data-select a").click(function() {
		$(this).children("input").prop("checked",true).parents("a").addClass('cur').siblings("a").removeClass('cur');
	});
	//多选模拟选择
	$(".data-moreselect a").click(function() {
		if(!$(this).hasClass('cur')){
			$(this).children("input").prop("checked",true).parents("a").addClass('cur');
		}else{
			$(this).children("input").prop("checked",false).parents("a").removeClass('cur');
		}
	});	
	...
	//日历默认
	/*$(document).on('blur', '.select-month', function() {
		if($(this).val() == ''){
			$(this).val($(this).data("value"));
		}
    });*/
	//体检查询下拉
	$('.tj_touch .am-g').on('click', function(){  
		$(this).next().toggle();
		$(this).find('.tjdown').toggleClass('tjup');
		});
	
	/*
	$('.gff_list > ul > li > a').on('touchstart', function(){  
		$(this).next().toggle();
		});	*/
	/**同级显示切换**/
	$('.data-siblings-tab').click(function() {
		if($(this).find(".am-fr").hasClass("b_link")){
			$(this).find(".am-fr").addClass("r_link").removeClass("b_link");
			$(this).siblings("ul").show();
		}else{
			$(this).find(".am-fr").addClass("b_link").removeClass("r_link");
	...
	$('.menu-list-item li.subli').click(function() {
		if(!$(this).hasClass('down')){
			...
			var e = e||event;
			if($(e.target).parents(".gsidlist").length==0){
